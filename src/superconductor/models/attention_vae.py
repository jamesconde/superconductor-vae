...
class AttentionBidirectionalVAE(nn.Module):
    """
    LEGACY (V1-V10) — Superseded by FullMaterialsVAE in V12.

    Original bidirectional VAE with integrated element attention.
    Used element-only encoding (no Magpie features, no Tc input).
    Kept for reference; not instantiated by any active training script.

    Replaced because: V12 added Magpie features + Tc as encoder inputs,
    requiring a fundamentally different encoder architecture (FullMaterialsVAE).
    """
...
class AttentionVAELoss(nn.Module):
    """
    LEGACY (V1-V10) — Loss function for AttentionBidirectionalVAE.

    Superseded by CombinedLossWithREINFORCE in train_v12_clean.py.
    Kept for reference; not instantiated by any active training script.
    """
...
def create_attention_vae(
    ...
    """LEGACY — Factory function for AttentionBidirectionalVAE. Not used by active code."""
...
class FullMaterialsLoss(nn.Module):
    """
    LEGACY (V12.0-V12.20) — Simple loss function for FullMaterialsVAE.

    Superseded by CombinedLossWithREINFORCE in train_v12_clean.py which adds
    REINFORCE, stoichiometry loss, type loss, stop loss, and dynamic weighting.
    Only referenced by the old scripts/train.py. Kept for reference.
    """
